...
#include <hbrs_srvs/GetPose.h>
#include <hbrs_msgs/LaserScanSegmentList.h>
#include <hbrs_algorithms/segmentation/laserscan_segmentation.h>
#include <hbrs_algorithms/geometry/conversions.h>
#include <float.h>
...
#include <hbrs_algorithms/wrapper/pcl_wrapper.hpp>
#include <tf/tf.h>
#include <laser_filters/angular_bounds_filter.h>
#include <math.h>


const std::string NODE_NAME = "hbrs_nearest_object_detector";
...
int main(int argc, char **argv)
{
    ...
    nh.getParam(NODE_NAME + "/PublishMarker", publish_marker);
	ROS_INFO_STREAM("\tpublish marker: " << publish_marker);

	//either do a single call or ...
	ros::ServiceServer srv_nearest_obj = nh.advertiseService(NODE_NAME + "/GetNearestObject", getNearestObject);

	// ... let the node continusly publishing the nearest obj on a topic
	ros::ServiceServer srv_stop = nh.advertiseService(NODE_NAME + "/stop", stop);
	ros::ServiceServer srv_start = nh.advertiseService(NODE_NAME + "/start", start);

	//Publisher
	pub_obj_pose = nh.advertise<geometry_msgs::PoseStamped>(NODE_NAME + "/nearest_object", 1);
