...
pcl::PointCloud<pcl::PointXYZRGBNormal> CToolBoxROS::movingLeastSquares(pcl::PointCloud<pcl::PointXYZRGB> &cloud, float searchRadius)
{
	...
	pcl::MovingLeastSquares<pcl::PointXYZRGB, pcl::Normal> movingLeastSquaresExtractor;
...
pcl::PointCloud<pcl::PointXYZ> CToolBoxROS::movingLeastSquares2(pcl::PointCloud<pcl::PointXYZ> &cloud, float searchRadius)
{
	...
	pcl::MovingLeastSquares<pcl::PointXYZ, pcl::Normal> movingLeastSquaresExtractor;
	movingLeastSquaresExtractor.setInputCloud(cloud.makeShared());
	movingLeastSquaresExtractor.setOutputNormals(mls_normals);
	movingLeastSquaresExtractor.setPolynomialFit(true);
	movingLeastSquaresExtractor.setSearchRadius(searchRadius);
...
pcl::PointCloud<pcl::PointNormal> CToolBoxROS::movingLeastSquares(pcl::PointCloud<pcl::PointXYZ> &cloud, float searchRadius)
{
	...
	/*for (unsigned int i = 0; i < mls_points.points.size(); ++i)
	 {
	 std::cout << mls_points.points[i] << "\n";
	 }*/
...
void CToolBoxROS::markClusteredPointCloud(std::vector<pcl::PointCloud<pcl::PointXYZRGBNormal> > &clusteredPointCloud, pcl::PointCloud<pcl::PointXYZRGBNormal> &markedPointCloud)
{
	int color = 0;
...
int CToolBoxROS::pointInsideConvexHull2d(pcl::PointCloud<pcl::PointXYZRGBNormal> convexHull, pcl::PointXYZRGBNormal point)
{
	...
	{
		if ((((convexHull.points[i].y <= point.y) && (point.y < convexHull.points[j].y)) || ((convexHull.points[j].y <= point.y) && (point.y < convexHull.points[i].y)))
				&& (point.x < (convexHull.points[j].x - convexHull.points[i].x) * (point.y - convexHull.points[i].y) / (convexHull.points[j].y - convexHull.points[i].y) + convexHull.points[i].x))
...
pcl::PointXYZRGBNormal CToolBoxROS::centroidHull2d(pcl::PointCloud<pcl::PointXYZRGBNormal> point_cloud, float area)
{
	...
	for (unsigned int iter_point = 0; iter_point < sizePointCloud - 1; iter_point++)
	{
		centroid.x += (point_cloud.points[iter_point].x + point_cloud.points[iter_point + 1].x)
				* (point_cloud.points[iter_point].x * point_cloud.points[iter_point + 1].y - point_cloud.points[iter_point + 1].x * point_cloud.points[iter_point].y);
...
void CToolBoxROS::get3DPointsWithinHull(const pcl::PointCloud<pcl::PointXYZRGBNormal> &cloudPCLFull, const pcl::PointCloud<pcl::PointXYZRGBNormal> &cloudPCLHull, const double dMinHeight,
